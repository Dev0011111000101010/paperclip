...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: (key: string) => string) {
  return windowKind === "lifetime" ? t("project.budget.window_lifetime") : t("project.budget.window_monthly");
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("project.budget.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("project.budget.of_limit", { percent: summary.utilizationPercent }) : t("project.budget.no_cap")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("project.budget.budget_label")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("project.budget.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("project.budget.soft_alert_at", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} pause` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("project.budget.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("project.budget.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("project.budget.paused_project")
          : t("project.budget.paused_scope")}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        ...
        <Input
          ...
          className="mt-2"
          inputMode="decimal"
          placeholder={t("project.budget.amount_placeholder")}
        />
      </div>
      <Button
        onClick={() => {
      ...
      >
        {isSaving ? t("project.budget.saving") : summary.amount > 0 ? t("project.budget.update_budget") : t("project.budget.set_budget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("project.budget.status_paused") : summary.status === "warning" ? t("project.budget.status_warning") : summary.status === "hard_stop" ? t("project.budget.status_hard_stop") : t("project.budget.status_healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("project.budget.error_invalid_amount")}</p>
        ) : null}
      </div>
    );
  }
