...
export function NewAgent() {
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("new_agent.failed_create"));
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      if (!isValidOpenCodeModelId(configValues.model)) {
        setFormError(t("new_agent.opencode_model_required"));
        return;
      }
    }
    createAgent.mutate(
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-lg font-semibold">{t("new_agent.title")}</h1>
        <p className="text-sm text-muted-foreground mt-1">
          {t("new_agent.subtitle")}
        </p>
      </div>
      ...
      <div className="border border-border">
        ...
        <div className="px-4 pt-4 pb-2">
          <input
            ...
            placeholder={t("config_form.fields.name_placeholder")}
            value={name}
        ...
        <div className="px-4 pb-2">
          <input
            ...
            placeholder={t("config_form.fields.title_placeholder")}
            value={title}
        ...
        <div className="border-t border-border px-4 py-4">
          <div className="space-y-3">
            <div>
              <h2 className="text-sm font-medium">{t("new_agent.company_skills")}</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("new_agent.company_skills_desc")}
              </p>
            </div>
            {availableSkills.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {t("new_agent.no_skills")}
              </p>
            ) : (
              <div className="space-y-3">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("new_agent.ceo_notice")}</p>
          )}
          {formError && (
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {testAgentState.pending ? t("new_agent.testing") : t("new_agent.test_agent")}
                </Button>
                <Button
                  size="sm"
                ...
                >
                  {createAgent.isPending ? t("new_agent.creating") : t("new_agent.create")}
                </Button>
              </div>
            </div>
          </div>
