  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>{t("experimental.confirm_title")}</DialogTitle>
          <DialogDescription>
            {preview
              ? t("experimental.confirm_task_count", { count, hours: preview.lookbackHours })
              : t("experimental.confirm_checking")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="max-h-[min(28rem,65vh)] space-y-3 overflow-y-auto pr-1">
          {preview && preview.items.length === 0 ? (
            <div className="rounded-md border border-border bg-muted/30 px-3 py-4 text-sm text-muted-foreground">
              {t("experimental.confirm_no_tasks")}
            </div>
          ) : null}
          ...
          {preview?.items.map((item) => (
            <div key={item.incidentKey} className="rounded-md border border-border bg-card px-3 py-3">
              ...
              <div className="mt-2 text-xs text-muted-foreground">
                {t("experimental.confirm_recovery_target")}{" "}
                <a
        ...
        {preview && preview.skippedOutsideLookback > 0 ? (
          <p className="text-xs text-muted-foreground">
            {t("experimental.confirm_skipped", { count: preview.skippedOutsideLookback })}
          </p>
        ) : null}

        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={isPending}>
            {t("experimental.confirm_cancel")}
          </Button>
          <Button variant="outline" onClick={onEnableOnly} disabled={isPending || !preview}>
            {t("experimental.confirm_enable_only")}
          </Button>
          <Button onClick={onEnableAndRun} disabled={isPending || !preview}>
            {count > 0 ? t("experimental.confirm_enable_create", { count }) : t("experimental.confirm_enable")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export function InstanceExperimentalSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("experimental.failed_update"));
    },
  });
  ...
  const previewMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("experimental.failed_preview"));
    },
  });
  ...
  const runRecoveryMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("experimental.failed_run"));
    },
  });
  ...
  function previewForEnable() {
    if (!lookbackHoursIsValid) {
      setActionError(t("experimental.lookback_hours_invalid"));
      return;
    }
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("experimental.environments_title")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("experimental.environments_desc")}
            </p>
          </div>
          <ToggleSwitch
            ...
            aria-label={t("experimental.environments_aria")}
          />
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("experimental.isolated_workspaces_title")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("experimental.isolated_workspaces_desc")}
            </p>
          </div>
          <ToggleSwitch
            ...
            disabled={toggleMutation.isPending}
            aria-label={t("experimental.isolated_workspaces_aria")}
          />
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("experimental.auto_restart_title")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("experimental.auto_restart_desc")}
            </p>
          </div>
          <ToggleSwitch
            ...
            disabled={toggleMutation.isPending}
            aria-label={t("experimental.auto_restart_aria")}
          />
        </div>
      </section>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex flex-col gap-5">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1.5">
              <h2 className="text-sm font-semibold">{t("experimental.auto_recovery_title")}</h2>
              <p className="max-w-2xl text-sm text-muted-foreground">
                {t("experimental.auto_recovery_desc")}
              </p>
            </div>
            <ToggleSwitch
              ...
              aria-label={t("experimental.auto_recovery_aria")}
            />
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-[minmax(10rem,14rem)_1fr] sm:items-end">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {t("experimental.run_now")}
              </Button>
            </div>
          </div>

          <p className="text-xs text-muted-foreground">
            {t("experimental.current_window", { count: lookbackHours })}
          </p>
        </div>
      </section>
