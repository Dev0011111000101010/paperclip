...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("import.preview.select_file")} />
    );
  }
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {conflicts.length} {t("import.conflicts.item")}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("import.conflicts.skipped") : t("import.conflicts.skip")}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("import.conflicts.confirmed")}
                      </>
                    ) : (
                      t("import.conflicts.confirm_rename")
                    )}
                  </button>
                )}
              </div>
...
function AdapterPickerList({
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">{t("import.adapters.title")}</h3>
          <span className="text-xs text-muted-foreground">
            {agents.length} {t("import.adapters.agent")}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
...
export function CompanyImport() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("import.empty.select_company")} />;
  }
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          <h2 className="text-base font-semibold">{t("import.section_title")}</h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("import.section_desc")}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("import.source.github") },
              { key: "local", icon: Upload, label: t("import.source.local") },
            ] as const
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            label={t("import.form.github_url")}
            hint={t("import.form.github_url_hint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder={t("import.form.github_url_placeholder")}
              onChange={(e) => {
        ...
        <Field label={t("import.form.target")} hint={t("import.form.target_hint")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{t("import.form.target_new")}</option>
            <option value="existing">
              {t("import.form.target_existing")} {selectedCompany?.name}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("import.form.new_company_name")}
            hint={t("import.form.new_company_hint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("import.form.new_company_placeholder")}
            />
          </Field>
        )}

        <Field
          label={t("import.form.collision_strategy")}
          hint={t("import.form.collision_strategy_hint")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("import.form.collision_rename")}</option>
            <option value="skip">{t("import.form.collision_skip")}</option>
            <option value="replace">{t("import.form.collision_replace")}</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? t("import.actions.previewing") : t("import.actions.preview")}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                {selectedCount} / {totalFiles} {totalFiles === 1 ? t("import.preview.file_selected") : t("import.preview.files_selected")}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {conflicts.length} {t("import.preview.conflicts")}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {importPreview.errors.length} {t("import.preview.errors")}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ...
                : t("import.actions.import") + ` ${selectedCount} file${selectedCount === 1 ? "" : "s"}`}
            </Button>
          </div>
          ...
          <div className="grid h-[calc(100vh-16rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
            <aside className="flex flex-col border-r border-border overflow-hidden">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("import.preview.package_files")}</h2>
              </div>
